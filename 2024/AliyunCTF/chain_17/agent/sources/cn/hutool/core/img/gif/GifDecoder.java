...
/* loaded from: agent.jar:BOOT-INF/lib/hutool-all-5.8.16.jar:cn/hutool/core/img/gif/GifDecoder.class */
public class GifDecoder {
    ...
    /* loaded from: agent.jar:BOOT-INF/lib/hutool-all-5.8.16.jar:cn/hutool/core/img/gif/GifDecoder$GifFrame.class */
        ...
        for (int i = 0; i < this.ih; i++) {
            ...
            if (line2 < this.height) {
                ...
                while (dx < dlim) {
                    int i2 = sx;
                    sx++;
                    int index = this.pixels[i2] & Const.MAX_ARRAY_DIMENSIONS;
    ...
    public int read(String name) {
        ...
        try {
            ...
            if (name2.contains("file:") || name2.indexOf(":/") > 0) {
                ...
                this.in = new BufferedInputStream(Files.newInputStream(Paths.get(name2, new String[0]), new OpenOption[0]));
            }
    ...
    protected int readBlock() {
        ...
        int n = 0;
        if (this.blockSize > 0) {
            while (n < this.blockSize && (count = this.in.read(this.block, n, this.blockSize - n)) != -1) {
                try {
                    n += count;
                } catch (IOException e) {
                }
