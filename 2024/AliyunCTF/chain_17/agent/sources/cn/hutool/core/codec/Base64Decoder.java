...
/* loaded from: agent.jar:BOOT-INF/lib/hutool-all-5.8.16.jar:cn/hutool/core/codec/Base64Decoder.class */
    ...
    public static byte[] decode(byte[] in, int pos, int length) {
        ...
        while (offset.intValue() <= maxPos) {
            ...
            if (-2 != sestet1) {
                int i = octetId;
                octetId++;
                octet[i] = (byte) ((sestet0 << 2) | (sestet1 >>> 4));
            }
            if (-2 != sestet2) {
                int i2 = octetId;
                octetId++;
                octet[i2] = (byte) (((sestet1 & 15) << 4) | (sestet2 >>> 2));
            }
            if (-2 != sestet3) {
                int i3 = octetId;
                octetId++;
                octet[i3] = (byte) (((sestet2 & 3) << 6) | sestet3);
            }
        }
