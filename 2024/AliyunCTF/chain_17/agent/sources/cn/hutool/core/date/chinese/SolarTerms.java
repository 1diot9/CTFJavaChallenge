...
/* loaded from: agent.jar:BOOT-INF/lib/hutool-all-5.8.16.jar:cn/hutool/core/date/chinese/SolarTerms.class */
public class SolarTerms {
    ...
    public static int getTerm(int y, int n) {
        ...
        String _table = S_TERM_INFO[y - LunarInfo.BASE_YEAR];
        ...
        for (int i2 = 0; i2 < 6; i2++) {
            _calday[4 * i2] = _info[i2].toString().substring(0, 1);
            _calday[(4 * i2) + 1] = _info[i2].toString().substring(1, 3);
            _calday[(4 * i2) + 2] = _info[i2].toString().substring(3, 4);
            _calday[(4 * i2) + 3] = _info[i2].toString().substring(4, 6);
        }
    ...
    private static String getTermInternal(int year, int mouth, int day) {
        ...
        String termTable = S_TERM_INFO[year - LunarInfo.BASE_YEAR];
