...
/* loaded from: agent.jar:BOOT-INF/lib/hutool-all-5.8.16.jar:cn/hutool/db/DbUtil.class */
public final class DbUtil {
    ...
    public static void setShowSqlGlobal(Setting setting) {
        boolean isShowSql = Convert.toBool(setting.remove(SqlLog.KEY_SHOW_SQL), false).booleanValue();
        boolean isFormatSql = Convert.toBool(setting.remove(SqlLog.KEY_FORMAT_SQL), false).booleanValue();
        boolean isShowParams = Convert.toBool(setting.remove(SqlLog.KEY_SHOW_PARAMS), false).booleanValue();
        ...
        log.debug("Show sql: [{}], format sql: [{}], show params: [{}], level: [{}]", Boolean.valueOf(isShowSql), Boolean.valueOf(isFormatSql), Boolean.valueOf(isShowParams), level);
