package cn.hutool.poi.excel.cell.setters;

import cn.hutool.poi.excel.cell.CellSetter;
import org.apache.poi.ss.usermodel.Cell;
import org.apache.poi.ss.usermodel.Hyperlink;

/* loaded from: agent.jar:BOOT-INF/lib/hutool-all-5.8.16.jar:cn/hutool/poi/excel/cell/setters/HyperlinkCellSetter.class */
public class HyperlinkCellSetter implements CellSetter {
    private final Hyperlink value;

    /* JADX INFO: Access modifiers changed from: package-private */
    public HyperlinkCellSetter(Hyperlink value) {
        this.value = value;
    }

    @Override // cn.hutool.poi.excel.cell.CellSetter
    public void setValue(Cell cell) {
        cell.setHyperlink(this.value);
        cell.setCellValue(this.value.getLabel());
    }
}
