...
/* loaded from: agent.jar:BOOT-INF/lib/hutool-all-5.8.16.jar:cn/hutool/poi/excel/ExcelUtil.class */
public class ExcelUtil {
    ...
    public static void readBySax(InputStream in, int rid, RowHandler rowHandler) {
        ...
        ExcelSaxReader<?> reader = ExcelSaxUtil.createSaxReader(ExcelFileUtil.isXlsx(in2), rowHandler);
        reader.read(in2, rid);
    }
    ...
    public static void readBySax(InputStream in, String idOrRidOrSheetName, RowHandler rowHandler) {
        InputStream in2 = IoUtil.toMarkSupportStream(in);
        ExcelSaxReader<?> reader = ExcelSaxUtil.createSaxReader(ExcelFileUtil.isXlsx(in2), rowHandler);
        reader.read(in2, idOrRidOrSheetName);
    }
    ...
    public static ExcelReader getReader(String bookFilePath, int sheetIndex) {
        try {
            ...
            throw new DependencyException((Throwable) ObjectUtil.defaultIfNull((NoClassDefFoundError) e.getCause(), e), PoiChecker.NO_POI_ERROR_MSG, new Object[0]);
        }
    }
