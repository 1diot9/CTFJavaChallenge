...
/* loaded from: agent.jar:BOOT-INF/lib/hutool-all-5.8.16.jar:cn/hutool/system/SystemUtil.class */
public class SystemUtil extends SystemPropsUtil {
    ...
    public static long getCurrentPID() {
        return Long.parseLong(getRuntimeMXBean().getName().split(StrPool.AT)[0]);
    }
    ...
    public static int getTotalThreadCount() {
        ...
        while (true) {
            ThreadGroup parentThread = threadGroup;
            if (null != parentThread.getParent()) {
                threadGroup = parentThread.getParent();
            } else {
    ...
    public static void append(StringBuilder builder, String caption, Object value) {
        builder.append(caption).append(StrUtil.nullToDefault(Convert.toStr(value), "[n/a]")).append(StrPool.LF);
    }
}
