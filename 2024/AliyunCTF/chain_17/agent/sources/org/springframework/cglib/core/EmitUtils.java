...
/* loaded from: agent.jar:BOOT-INF/lib/spring-core-6.1.3.jar:org/springframework/cglib/core/EmitUtils.class */
public class EmitUtils {
    ...
    public static final ArrayDelimiters DEFAULT_DELIMITERS = new ArrayDelimiters(StrPool.DELIM_START, ", ", "}");

    /* JADX INFO: Access modifiers changed from: private */
    /* loaded from: agent.jar:BOOT-INF/lib/spring-core-6.1.3.jar:org/springframework/cglib/core/EmitUtils$ParameterTyper.class */
    ...
    private static void string_switch_trie(final CodeEmitter e, String[] strings, final ObjectSwitchCallback callback) throws Exception {
        ...
        final Map buckets = CollectionUtils.bucket(Arrays.asList(strings), value -> {
            return Integer.valueOf(((String) value).length());
        });
        e.dup();
        e.invoke_virtual(Constants.TYPE_STRING, STRING_LENGTH);
        e.process_switch(getSwitchKeys(buckets), new ProcessSwitchCallback() { // from class: org.springframework.cglib.core.EmitUtils.1
    ...
    private static void stringSwitchHelper(final CodeEmitter e, List strings, final ObjectSwitchCallback callback, final Label def, final Label end, final int index) throws Exception {
        ...
        final Map buckets = CollectionUtils.bucket(strings, value -> {
            return Character.valueOf(((String) value).charAt(index));
        });
        e.dup();
        e.push(index);
    ...
    private static void string_switch_hash(final CodeEmitter e, final String[] strings, final ObjectSwitchCallback callback, final boolean skipEquals) throws Exception {
        ...
        e.process_switch(getSwitchKeys(buckets), new ProcessSwitchCallback() { // from class: org.springframework.cglib.core.EmitUtils.3
            @Override // org.springframework.cglib.core.ProcessSwitchCallback
            public void processCase(int key, Label ignore_end) throws Exception {
                List bucket = (List) buckets.get(Integer.valueOf(key));
                Label next = null;
    ...
    public static void push_array(CodeEmitter e, Object[] array) {
        e.push(array.length);
        e.newarray(Type.getType((Class<?>) remapComponentType(array.getClass().componentType())));
    ...
    private static void hash_array(final CodeEmitter e, Type type, final int multiplier, final CustomizerRegistry registry) {
        ...
        e.dup();
        e.ifnull(skip);
        process_array(e, type, type1 -> {
            hash_code(e, type1, multiplier, registry);
        });
        e.goTo(end);
        e.mark(skip);
        e.pop();
    ...
    public static void not_equals(final CodeEmitter e, Type type, final Label notEquals, final CustomizerRegistry registry) {
        new ProcessArrayCallback() { // from class: org.springframework.cglib.core.EmitUtils.4
            ...
            public void processElement(Type type2) {
                EmitUtils.not_equals_helper(CodeEmitter.this, type2, notEquals, registry, this);
            }
        }.processElement(type);
    }
    ...
    public static void append_string(final CodeEmitter e, Type type, final ArrayDelimiters delims, final CustomizerRegistry registry) {
        ...
        ProcessArrayCallback callback = new ProcessArrayCallback() { // from class: org.springframework.cglib.core.EmitUtils.5
            @Override // org.springframework.cglib.core.ProcessArrayCallback
            public void processElement(Type type2) {
                EmitUtils.append_string_helper(CodeEmitter.this, type2, d, registry, this);
                CodeEmitter.this.push(d.inside);
    ...
    /* loaded from: agent.jar:BOOT-INF/lib/spring-core-6.1.3.jar:org/springframework/cglib/core/EmitUtils$ArrayDelimiters.class */
        ...
        try {
            ...
            final ParameterTyper cached = member -> {
                ...
                if (types == null) {
                    ...
                    types = argumentTypes;
                    cache.put(member, argumentTypes);
                }
                return types;
            };
            final Label def = e.make_label();
            ...
            if (useName) {
                e.swap();
                final Map buckets = CollectionUtils.bucket(members, value -> {
                    return ((MethodInfo) value).getSignature().getName();
                });
                String[] names = (String[]) buckets.keySet().toArray(new String[buckets.size()]);
                string_switch(e, names, 1, new ObjectSwitchCallback() { // from class: org.springframework.cglib.core.EmitUtils.6
                    @Override // org.springframework.cglib.core.ObjectSwitchCallback
                    public void processCase(Object key, Label dontUseEnd) throws Exception {
                        EmitUtils.member_helper_size(CodeEmitter.this, (List) buckets.get(key), callback, cached, def, end);
                    }

                    @Override // org.springframework.cglib.core.ObjectSwitchCallback
                    public void processDefault() throws Exception {
                        CodeEmitter.this.goTo(def);
                    }
                });
            } else {
    ...
    private static void member_helper_size(final CodeEmitter e, List members, final ObjectSwitchCallback callback, final ParameterTyper typer, final Label def, final Label end) throws Exception {
        ...
        e.dup();
        e.arraylength();
        e.process_switch(getSwitchKeys(buckets), new ProcessSwitchCallback() { // from class: org.springframework.cglib.core.EmitUtils.7
            @Override // org.springframework.cglib.core.ProcessSwitchCallback
    ...
    private static void member_helper_type(final CodeEmitter e, List members, final ObjectSwitchCallback callback, final ParameterTyper typer, final Label def, final Label end, final BitSet checked) throws Exception {
        ...
        for (int i2 = 0; i2 < example.length; i2++) {
            int j = i2;
            Map test = CollectionUtils.bucket(members, value -> {
        ...
        String[] names = (String[]) buckets.keySet().toArray(new String[buckets.size()]);
        string_switch(e, names, 1, new ObjectSwitchCallback() { // from class: org.springframework.cglib.core.EmitUtils.8
            @Override // org.springframework.cglib.core.ObjectSwitchCallback
            public void processCase(Object key, Label dontUseEnd) throws Exception {
                EmitUtils.member_helper_type(CodeEmitter.this, (List) fbuckets.get(key), callback, typer, def, end, checked);
            }
