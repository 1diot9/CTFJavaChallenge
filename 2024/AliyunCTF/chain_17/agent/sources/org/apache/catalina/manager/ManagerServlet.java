...
/* loaded from: agent.jar:BOOT-INF/lib/tomcat-embed-core-10.1.18.jar:org/apache/catalina/manager/ManagerServlet.class */
public class ManagerServlet extends HttpServlet implements ContainerServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (path != null) {
            ...
            cn2 = ContextName.extractFromPath(config);
        } else if (war != null) {
            cn2 = ContextName.extractFromPath(war);
        }
        ...
        if (command == null) {
            ...
            if (war != null || config != null) {
                deploy(writer, config, cn2, war, update, smClient);
            } else if (tag != null) {
                deploy(writer, cn2, tag, smClient);
            } else {
            ...
            expireSessions(writer, cn2, request, smClient);
        } else if (command.equals("/expire")) {
            expireSessions(writer, cn2, request, smClient);
        } else if (command.equals("/start")) {
            start(writer, cn2, smClient);
        } else if (command.equals("/stop")) {
            stop(writer, cn2, smClient);
        } else if (command.equals("/undeploy")) {
            undeploy(writer, cn2, smClient);
    ...
    public void doPut(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (command == null) {
            ...
            deploy(writer, config, cn2, tag, update, request, smClient);
        } else {
            writer.println(smClient.getString("managerServlet.unknownCommand", command));
        }
        writer.flush();
    ...
    public void init() throws ServletException {
        ...
        this.versioned = (File) getServletContext().getAttribute("jakarta.servlet.context.tempdir");
    ...
    protected void deploy(PrintWriter writer, String config, ContextName cn2, String tag, boolean update, HttpServletRequest request, StringManager smClient) {
        File uploadedWar;
        ...
        if (this.debug >= 1) {
            if (config == null) {
                log("deploy: Deploying web application '" + cn2 + "'");
            } else {
                log("deploy: Deploying web application '" + cn2 + "' with context configuration at '" + config + "'");
            }
        }
        if (validateContextName(cn2, writer, smClient)) {
            String name = cn2.getName();
            String baseName = cn2.getBaseName();
            String displayPath = cn2.getDisplayName();
                    ...
                    if (config != null) {
                        try {
                            if (!this.configBase.mkdirs() && !this.configBase.isDirectory()) {
                                ...
                                removeServiced(name);
                                return;
                            } else if (!ExpandWar.copy(new File(config), new File(this.configBase, baseName + ".xml"))) {
    ...
    protected void deploy(PrintWriter writer, ContextName cn2, String tag, StringManager smClient) {
        if (!validateContextName(cn2, writer, smClient)) {
            return;
        }
        String baseName = cn2.getBaseName();
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    /* JADX WARN: Finally extract failed */
    public void deploy(PrintWriter writer, String config, ContextName cn2, String war, boolean update, StringManager smClient) {
        if (config != null && config.length() == 0) {
            config = null;
        }
        ...
        if (this.debug >= 1) {
            if (config != null) {
                ...
                log("install: Installing web application '" + cn2 + "' from '" + war + "'");
            } else {
                ...
                if (tryAddServiced(name)) {
                    ...
                    if (war != null) {
                        ...
                        if (!warFile.isAbsolute()) {
                            warFile = new File(this.host.getAppBaseFile(), war);
                        }
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public void reload(PrintWriter writer, ContextName cn2, StringManager smClient) {
        if (this.debug >= 1) {
            log("restart: Reloading web application '" + cn2 + "'");
        }
        if (!validateContextName(cn2, writer, smClient)) {
            return;
        }
        try {
            Context context = (Context) this.host.findChild(cn2.getName());
            if (context == null) {
                writer.println(smClient.getString("managerServlet.noContext", Escape.htmlElementContent(cn2.getDisplayName())));
            } else if (context.getName().equals(this.context.getName())) {
                writer.println(smClient.getString("managerServlet.noSelf"));
            } else {
                context.reload();
                writer.println(smClient.getString("managerServlet.reloaded", cn2.getDisplayName()));
            }
        } catch (Throwable t) {
            ExceptionUtils.handleThrowable(t);
            log(sm.getString("managerServlet.error.reload", cn2.getDisplayName()), t);
            writer.println(smClient.getString("managerServlet.exception", t.toString()));
        }
    }
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public void sessions(PrintWriter writer, ContextName cn2, int idle, StringManager smClient) {
        if (this.debug >= 1) {
            log("sessions: Session information for web application '" + cn2 + "'");
            if (idle >= 0) {
                log("sessions: Session expiration for " + idle + " minutes '" + cn2 + "'");
            }
        }
        if (!validateContextName(cn2, writer, smClient)) {
            return;
    ...
    protected void expireSessions(PrintWriter writer, ContextName cn2, HttpServletRequest req, StringManager smClient) {
        int idle = -1;
        ...
        sessions(writer, cn2, idle, smClient);
    }

    /* JADX INFO: Access modifiers changed from: protected */
    public void start(PrintWriter writer, ContextName cn2, StringManager smClient) {
        if (this.debug >= 1) {
            log("start: Starting web application '" + cn2 + "'");
        }
        if (!validateContextName(cn2, writer, smClient)) {
            return;
        }
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public void stop(PrintWriter writer, ContextName cn2, StringManager smClient) {
        if (this.debug >= 1) {
            log("stop: Stopping web application '" + cn2 + "'");
        }
        if (!validateContextName(cn2, writer, smClient)) {
            return;
        }
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public void undeploy(PrintWriter writer, ContextName cn2, StringManager smClient) {
        if (this.debug >= 1) {
            log("undeploy: Undeploying web application at '" + cn2 + "'");
        }
        if (validateContextName(cn2, writer, smClient)) {
            String name = cn2.getName();
            String baseName = cn2.getBaseName();
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public boolean tryAddServiced(String name) throws Exception {
        String[] params = {name};
        String[] signature = {"java.lang.String"};
    ...
    protected static boolean validateContextName(ContextName cn2, PrintWriter writer, StringManager smClient) {
        if (cn2 != null && (cn2.getPath().startsWith("/") || cn2.getPath().equals(""))) {
            return true;
        }
        String path = null;
        if (cn2 != null) {
            path = Escape.htmlElementContent(cn2.getPath());
        }
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public Map<String, List<String>> getConnectorCiphers(StringManager smClient) {
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public Map<String, List<String>> getConnectorCerts(StringManager smClient) {
        Map<String, List<String>> result = new HashMap<>();
        Connector[] connectors = getConnectors();
        for (Connector connector : connectors) {
            if (Boolean.TRUE.equals(connector.getProperty("SSLEnabled"))) {
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public Map<String, List<String>> getConnectorTrustedCerts(StringManager smClient) {
        Map<String, List<String>> result = new HashMap<>();
        Connector[] connectors = getConnectors();
        for (Connector connector : connectors) {
            if (Boolean.TRUE.equals(connector.getProperty("SSLEnabled"))) {
