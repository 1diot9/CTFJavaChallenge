...
/* loaded from: server.jar:BOOT-INF/lib/jooq-3.19.3.jar:org/jooq/impl/SelectIsNull.class */
final class SelectIsNull extends AbstractCondition implements QOM.SelectIsNull {
    ...
    /* loaded from: server.jar:BOOT-INF/lib/jooq-3.19.3.jar:org/jooq/impl/SelectIsNull$1.class */
    ...
    public final void accept(Context<?> ctx) {
        if (EMULATE_NULL_QUERY.contains(ctx.dialect())) {
            if (this.select.getSelect().size() == 1) {
                ...
                ctx.visit(DSL.inline(1).eq(DSL.selectCount().from(as).where(Tools.allNull(as.fields()))));
                return;
            }
        }
        acceptStandard(ctx);
