...
/* loaded from: server.jar:BOOT-INF/lib/jooq-3.19.3.jar:org/jooq/impl/Right.class */
public final class Right extends AbstractField<String> implements QOM.Right {
    final Field<String> string;
    final Field<? extends Number> length;

    /* JADX INFO: Access modifiers changed from: package-private */
    public Right(Field<String> string, Field<? extends Number> length) {
    ...
    public final void accept(Context<?> ctx) {
        switch (ctx.family()) {
            case DERBY:
                ctx.visit((Field<?>) DSL.substring(this.string, (Field<? extends Number>) Internal.iadd(DSL.length(this.string), Internal.isub(DSL.one(), this.length))));
                return;
            case SQLITE:
            case TRINO:
                ctx.visit((Field<?>) DSL.substring(this.string, (Field<? extends Number>) Internal.ineg(this.length)));
                return;
            default:
                ctx.visit(DSL.function(Names.N_RIGHT, getDataType(), (Field<?>[]) new Field[]{this.string, this.length}));
                return;
        }
    }
