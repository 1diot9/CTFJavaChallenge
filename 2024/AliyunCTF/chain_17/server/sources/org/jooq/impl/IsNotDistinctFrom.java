...
/* loaded from: server.jar:BOOT-INF/lib/jooq-3.19.3.jar:org/jooq/impl/IsNotDistinctFrom.class */
public final class IsNotDistinctFrom<T> extends AbstractCondition implements QOM.IsNotDistinctFrom<T> {
    final Field<T> arg1;
    final Field<T> arg2;

    /* JADX INFO: Access modifiers changed from: package-private */
    public IsNotDistinctFrom(Field<T> arg1, Field<T> arg2) {
    ...
    public final void accept(Context<?> ctx) {
        if (this.arg1.getDataType().isEmbeddable() && this.arg2.getDataType().isEmbeddable()) {
            ctx.visit(DSL.row((SelectField<?>[]) Tools.embeddedFields((Field<?>) this.arg1)).isNotDistinctFrom(DSL.row((SelectField<?>[]) Tools.embeddedFields((Field<?>) this.arg2))));
            return;
        }
        if (IsDistinctFrom.EMULATE_DISTINCT_PREDICATE.contains(ctx.dialect())) {
            ctx.visit(DSL.exists(DSL.select(this.arg1.as("x")).intersect((Select) DSL.select(this.arg2.as("x")))));
            return;
        }
        if (IsDistinctFrom.SUPPORT_DISTINCT_WITH_ARROW.contains(ctx.dialect())) {
            ...
            ctx.visit((Field<?>) this.arg1).sql(' ').visit(Keywords.K_IS).sql(' ').visit(Keywords.K_NOT).sql(' ').visit(Keywords.K_DISTINCT).sql(' ').visit(Keywords.K_FROM).sql(' ').visit((Field<?>) this.arg2);
        }
    }
    ...
    public final Function2<? super Field<T>, ? super Field<T>, ? extends QOM.IsNotDistinctFrom<T>> $constructor() {
        return (a1, a2) -> {
            return new IsNotDistinctFrom(a1, a2);
        };
    }
