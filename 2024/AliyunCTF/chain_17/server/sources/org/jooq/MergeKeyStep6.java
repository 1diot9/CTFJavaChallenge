...
/* loaded from: server.jar:BOOT-INF/lib/jooq-3.19.3.jar:org/jooq/MergeKeyStep6.class */
public interface MergeKeyStep6<R extends Record, T1, T2, T3, T4, T5, T6> extends MergeValuesStep6<R, T1, T2, T3, T4, T5, T6> {
    ...
    @Deprecated(forRemoval = true, since = "3.14")
    @NotNull
    MergeValuesStep6<R, T1, T2, T3, T4, T5, T6> key(Field<?>... fieldArr);

    @Support({SQLDialect.CUBRID, SQLDialect.DERBY, SQLDialect.FIREBIRD, SQLDialect.H2, SQLDialect.HSQLDB, SQLDialect.POSTGRES, SQLDialect.YUGABYTEDB})
    @CheckReturnValue
    @Deprecated(forRemoval = true, since = "3.14")
    @NotNull
    MergeValuesStep6<R, T1, T2, T3, T4, T5, T6> key(Collection<? extends Field<?>> collection);
}
