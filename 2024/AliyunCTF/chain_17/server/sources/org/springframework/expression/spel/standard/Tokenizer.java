...
/* loaded from: server.jar:BOOT-INF/lib/spring-expression-6.1.3.jar:org/springframework/expression/spel/standard/Tokenizer.class */
class Tokenizer {
        ...
        while (this.pos < this.max) {
            ...
            if (isAlphabetic(ch2)) {
                ...
                switch (ch2) {
                    ...
                    case 'z':
                    default:
                        throw new IllegalStateException("Unsupported character '%s' (%d) encountered at position %d in expression.".formatted(Character.valueOf(ch2), Integer.valueOf(ch2), Integer.valueOf(this.pos + 1)));
                    case '\t':
                    case '\n':
                    case '\r':
                    case ' ':
