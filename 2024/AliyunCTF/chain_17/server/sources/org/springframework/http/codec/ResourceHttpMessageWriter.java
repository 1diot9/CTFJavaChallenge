...
/* loaded from: server.jar:BOOT-INF/lib/spring-web-6.1.3.jar:org/springframework/http/codec/ResourceHttpMessageWriter.class */
public class ResourceHttpMessageWriter implements HttpMessageWriter<Resource> {
    ...
    private Mono<Void> writeResource(Resource resource, ResolvableType type, @Nullable MediaType mediaType, ReactiveHttpOutputMessage message, Map<String, Object> hints) {
        return addDefaultHeaders(message, resource, mediaType, hints).then(Mono.defer(() -> {
            ...
            Flux<DataBuffer> body = this.encoder.encode((Publisher) input, factory, type, (MimeType) message.getHeaders().getContentType(), (Map<String, Object>) hints).subscribeOn(Schedulers.boundedElastic());
    ...
    public Mono<Void> addDefaultHeaders(ReactiveHttpOutputMessage message, Resource resource, @Nullable MediaType contentType, Map<String, Object> hints) {
        return Mono.defer(() -> {
            ...
            if (headers.getContentLength() < 0) {
                return lengthOf(resource).flatMap(contentLength -> {
    ...
    private static Mono<Long> lengthOf(Resource resource) {
        if (InputStreamResource.class != resource.getClass()) {
            ...
            return Mono.fromCallable(resource::contentLength).filter(length -> {
                return length.longValue() != -1;
            }).onErrorComplete(IOException.class).subscribeOn(Schedulers.boundedElastic());
        }
        return Mono.empty();
    }

    @Nullable
    private static Mono<Void> zeroCopy(Resource resource, @Nullable ResourceRegion region, ReactiveHttpOutputMessage message, Map<String, Object> hints) {
        if (!(message instanceof ZeroCopyHttpOutputMessage)) {
            return null;
        }
        ZeroCopyHttpOutputMessage zeroCopyHttpOutputMessage = (ZeroCopyHttpOutputMessage) message;
        if (resource.isFile()) {
            try {
                ...
                if (logger.isDebugEnabled()) {
                    String formatted = region != null ? "region " + pos + "-" + pos + " of " : "";
            ...
            return Mono.from(inputStream).flatMap(resource -> {
                ...
                if (regions.size() == 1) {
                    ...
                    return lengthOf(resource).flatMap(contentLength -> {
                        ...
                        long end = Math.min((start + region.getCount()) - 1, contentLength.longValue() - 1);
                        headers.add(HttpHeaders.CONTENT_RANGE, "bytes " + start + "-" + headers + "/" + end);
    ...
    private Mono<Void> encodeAndWriteRegions(Publisher<? extends ResourceRegion> publisher, @Nullable MediaType mediaType, ReactiveHttpOutputMessage message, Map<String, Object> hints) {
        Flux<DataBuffer> body = this.regionEncoder.encode(publisher, message.bufferFactory(), REGION_TYPE, mediaType, hints).subscribeOn(Schedulers.boundedElastic());
        return message.writeWith(body);
    }
}
